...
在项目的 `CLAUDE.md` 或 `AGENTS.md` 中添加简短指引即可。**无需硬编码路径** — AI 搜索 `capture.ps1` 即可定位工具目录。

<details>
<summary><b>CLAUDE.md / AGENTS.md</b> 示例</summary>

```markdown
## ProcMon 自动化采集与分析

本项目包含 ProcMon AI Toolkit，用于监控运行时的 DLL 加载、文件访问和注册表操作。
**仅在需要诊断运行时行为时调用** — 如排查 Init 失败、验证 DLL 搜索路径、分析文件 I/O 模式等。
不要在每个任务中都运行 ProcMon，它是按需使用的诊断工具。

搜索 `capture.ps1` 定位工具目录。MCP 分析工具已在 `.mcp.json` 中配置。

- 采集：`& "<工具目录>/capture.ps1" -TargetCommand "<命令>"` （自动按进程名过滤）
- 过滤器：`& $py "<工具目录>/gen-filter.py" -o filter.pmc --process X.exe [--path-contains Y]`
