...
```mermaid
graph TD
    AI["🤖 AI 智能体<br/>(Claude Code, Copilot, Cursor 等)"]
    ...
    subgraph MCP["ProcmonMCP — MCP 服务器"]
        M1["load_file"]
        M2["query_events"]
        M3["find_file_access"]
        M4["list_processes"]
    ...
    FILTER["gen-filter.py<br/><i>进程名 · 路径 · 操作类型 · 结果</i>"]
    FILTER -->|".pmc 过滤器"| CAPTURE
...
在项目的 `codex.json`（或全局 `~/.codex/config.json`）中添加：
...
在项目的指令文件中添加以下内容，让 AI 智能体**自动发现并使用**本工具。

<details>
<summary><b>CLAUDE.md</b>（适用于 Claude Code）</summary>

```markdown
## ProcMon 自动化采集与分析

本项目配置了 ProcMon 自动化工具链（`04_tools/procmon-mcp`），AI 可自主完成"采集 → 导出 → 分析"
全链路，无需用户人工介入。

使用场景：SDK Init 文件访问分析、DLL 加载链验证、运行根目录依赖排查等。

### 采集（PowerShell 脚本）

\`\`\`powershell
# 基本用法（自动按目标进程名过滤）
& "path/to/capture.ps1" -TargetCommand "<目标命令>"

# 指定进程名 / 自定义 PMC / 超时
& "path/to/capture.ps1" -TargetCommand "..." -ProcessName "MyApp.exe"
& "path/to/capture.ps1" -TargetCommand "..." -FilterConfig "<PMC路径>"
& "path/to/capture.ps1" -TargetCommand "..." -TimeoutSeconds 60
\`\`\`

脚本自动完成：生成过滤器 → 启动 ProcMon → 运行目标命令 → 停止 ProcMon → 导出 XML。
需要更精细的过滤条件时，先用 `gen-filter.py` 生成 PMC，再传给 `capture.ps1 -FilterConfig`。

### 分析（MCP 工具，.mcp.json 已配置）

1. `load_file` — 加载采集产出的 XML
2. `query_events` — 按进程/操作/路径/结果过滤
3. `find_file_access` — 按路径搜索文件访问
4. `list_processes` / `get_process_details` — 进程信息
5. `count_events_by_process` / `summarize_operations_by_process` — 统计
6. `export_query_results` — 导出 CSV/JSON

### 约束

- 采集产出不入 Git（`.gitignore` 已排除 `captures/`）。
- MCP 分析是只读操作，不修改任何文件。
```
...
<details>
<summary><b>AGENTS.md</b>（适用于 Codex / 多智能体协作）</summary>

```markdown
## ProcMon 自动化采集与分析

本项目配置了 ProcMon 自动化工具链，AI 可自主完成"采集 → 导出 → 分析"全链路。

- 工具目录：`04_tools/procmon-mcp`（独立 Python venv + 采集脚本）
- MCP 配置：项目根目录 `.mcp.json`（`procmon` 服务器）

### 第一步：采集

调用 `capture.ps1` 自动完成 ProcMon 启动、目标运行、停止和 XML 导出。
脚本默认自动提取目标命令的进程名作为过滤器。
...
# 指定进程名过滤
& "path/to/capture.ps1" -TargetCommand "<目标命令>" -ProcessName "MyApp.exe"

# 使用自定义 PMC 过滤器（高级场景）
& "path/to/capture.ps1" -TargetCommand "<目标命令>" -FilterConfig "<PMC路径>"
\`\`\`

产出在 `04_tools/procmon-mcp/captures/capture-<时间戳>/`：
- `capture.xml` — ProcMon 事件 XML（供 MCP 加载）
- `manifest.json` — 元数据（目标退出码、stdout/stderr、耗时、文件大小）
...
1. `load_file` — 加载 `capture.xml`
2. `query_events` — 按进程名、操作类型、路径、结果过滤事件
3. `find_file_access` — 按路径子串搜索文件访问
4. `list_processes` / `get_process_details` — 进程信息和模块列表
5. `count_events_by_process` / `summarize_operations_by_process` — 统计分析
6. `get_timing_statistics` / `get_process_lifetime` — 时序分析
7. `export_query_results` — 导出查询结果为 CSV 或 JSON

### 约束

- 采集产出不入 Git。
- `capture.ps1` 只做 ProcMon 采集控制，不运行任意应用、不连接设备。
- MCP 分析是只读操作。
```
