...
在项目的 `CLAUDE.md` 或 `AGENTS.md` 中添加简短指引，让 AI 智能体自动发现本工具。保持精简 — 详细用法由 AI 读本 README 获取。

<details>
<summary><b>CLAUDE.md</b> 示例</summary>

```markdown
## ProcMon 自动化采集与分析

ProcMon 自动化工具链位于 `04_tools/procmon-mcp`，AI 可自主完成采集和分析。

- 采集：`& "path/to/capture.ps1" -TargetCommand "<命令>"` （自动按进程名过滤）
- 过滤器：`& $py gen-filter.py -o filter.pmc --process X.exe [--path-contains Y] [--operation Z]`
- 分析：MCP 工具 — `load_file` → `query_events` / `find_file_access` / `count_events_by_process`
- 详见：`04_tools/procmon-mcp/README.md`

采集产出不入 Git。MCP 分析是只读操作。
```

</details>

<details>
<summary><b>AGENTS.md</b> 示例</summary>

```markdown
## ProcMon 自动化采集与分析

ProcMon 自动化工具链位于 `04_tools/procmon-mcp`，MCP 配置在 `.mcp.json`。
